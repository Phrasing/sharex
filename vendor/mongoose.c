...
void mg_http_serve_dir(struct mg_connection *c, struct mg_http_message *hm,
  ...
  auto *filtered_path = ::mg_remove_double_dots(path);
  if (flags < 0) {
    ...
    mg_http_reply(c, 404, opts->extra_headers, "Not found\n");
             ...
             mg_globmatch(sp, strlen(sp), filtered_path,
                          strlen(filtered_path))) {
    mg_http_serve_ssi(c, opts->root_dir, filtered_path);
  } else {
    mg_http_serve_file(c, hm, filtered_path, opts);
  }
}
